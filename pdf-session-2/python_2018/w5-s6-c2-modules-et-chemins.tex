...
\KeywordTok{def}\NormalTok{ show_argv_and_path():}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"le répertoire courant est }\SpecialCharTok{\{}\NormalTok{Path}\SpecialCharTok{.}\NormalTok{cwd()}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"le point d'entrée du programme est }\SpecialCharTok{\{}\NormalTok{sys}\SpecialCharTok{.}\NormalTok{argv[}\DecValTok{0}\NormalTok{]}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"la variable sys.path contient"}\NormalTok{)}
    \ControlFlowTok{for}\NormalTok{ i, path }\KeywordTok{in} \BuiltInTok{enumerate}\NormalTok{(sys.path, }\DecValTok{1}\NormalTok{):}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{i\}}\SpecialStringTok{-ème chemin dans sys.path }\SpecialCharTok{\{}\NormalTok{path}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}

\NormalTok{show_argv_and_path()}
\end{Highlighting}
\end{Shaded}
...
alors vous devriez observer ceci~:
    ...
    \begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\NormalTok{$ }\BuiltInTok{cd}\NormalTok{ /le/repertoire/ou/vous/etes/}
\ExtensionTok{/le/repertoire/ou/vous/etes}\NormalTok{ $ python3 /le/repertoire/du/script/run.py}
\ExtensionTok{le}\NormalTok{ répertoire courant est /le/repertoire/ou/vous/etes}
\ExtensionTok{le}\NormalTok{ point d}\StringTok{'entrée du programme est /le/repertoire/du/script/run.py}
\StringTok{la variable sys.path contient}
\StringTok{1-ème chemin dans sys.path /le/repertoire/du/script}
\StringTok{<snip>}
\end{Highlighting}
\end{Shaded}
...
dans~:
    ...
    \begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\ExtensionTok{/usr/share/utilitaire/}
                      \ExtensionTok{main.py}
                      \ExtensionTok{spam.py}
                      \ExtensionTok{eggs.py}
\end{Highlighting}
\end{Shaded}
...
contient juste
    ...
    \begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\ImportTok{import}\NormalTok{ spam, eggs}
\end{Highlighting}
\end{Shaded}
...
comme~:
    ...
    \begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\CommentTok{# on récupère le répertoire où est installé le point d'entrée}
\ImportTok{from}\NormalTok{ pathlib }\ImportTok{import}\NormalTok{ Path}

\NormalTok{directory_installation }\OperatorTok{=}\NormalTok{ Path(}\VariableTok{__file__}\NormalTok{).parent}
    
\CommentTok{# et on l'ajoute au chemin de recherche des modules}
\ImportTok{import}\NormalTok{ sys}
\NormalTok{sys.path.append(directory_installation)}
    
\CommentTok{# maintenant on peut importer spam et eggs de n'importe où}
\ImportTok{import}\NormalTok{ spam, eggs}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\ExtensionTok{pip3}\NormalTok{ install setuptools}
\end{Highlighting}
\end{Shaded}
