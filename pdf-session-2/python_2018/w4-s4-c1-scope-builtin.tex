...
\ImportTok{from}\NormalTok{ builtins }\ImportTok{import} \OperatorTok{*}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\OperatorTok{>>>} \KeywordTok{lambda} \OperatorTok{=} \DecValTok{1}
\NormalTok{  File }\StringTok{"<stdin>"}\NormalTok{, line }\DecValTok{1}
    \KeywordTok{lambda} \OperatorTok{=} \DecValTok{1}
           \OperatorTok{^}
