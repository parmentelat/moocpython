...
\ControlFlowTok{for}\NormalTok{ item }\KeywordTok{in}\NormalTok{ container:}
\NormalTok{    do_something(item)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[frame=lines,framerule=0.6mm,rulecolor=\color{asisframecolor}]
\KeywordTok{def}\NormalTok{ foo(graphic):}
\NormalTok{    ...}
\NormalTok{    graphic.draw()}
\end{Highlighting}
\end{Shaded}
